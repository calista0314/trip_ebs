...
class TripEbsTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            cols = list(dict.fromkeys(j for i in data for j in i))
            
            required =['公司段编码', '科目段编码', '子目段编码', '部门段编码', 'BU段编码', '产品分类段编码', '往来段编码', 'BU往来段编码', '项目段编码', '备用段1编码', '备用段2编码']

            cols_str = set(cols) & set(required)
            
            df = pd.read_json(StringIO(json_str), dtype= {i: str for i in cols_str})
        ...
        try:  
            ...
            cols_required = set(required) - set(cols)
            
            for i in cols_required:    
            ...
            df_ebs_template['类别'] = df_ebs_template['类别'].fillna('记账')
